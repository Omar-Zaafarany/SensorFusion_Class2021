...
void clusterLidarWithROI(std::vector<BoundingBox> &boundingBoxes, std::vector<LidarPoint> &lidarPoints)
{
    ...
    cv::Mat X(4, 1, cv::DataType<double>::type); // [x,y,z,1]
    cv::Mat Y(3, 1, cv::DataType<double>::type); // [x^, y^, w] // u = x^/w, v = y^/w

//////////////////////////////////////////////////////////////////////////////////////////////
    // Loop over lidar points
    //      project lidar point to image
    //      newList of enclosingBoxes
    //      Loop over bounding boxes
    //          if currentBoundingbox contains lidar point
    //              enclosingBoxes.pushback(currentBoundingbox) 
    //      if lidar point has been enclosed by only one boundingbox
...
int main()
{
    ...
    {
        if (it->lidarPoints.size() > 0) // if there is any lidar point in the BoundaingBox
        {
            showLidarTopview(it->lidarPoints, cv::Size(10.0, 25.0), cv::Size(500, 750));
        }
    }   
